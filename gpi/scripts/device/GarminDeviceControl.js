...
//		}
     ...
     */	
	// TODO remove the unused filename parameter in a future compatibility release.     
     ...
     */	
     // TODO Rename this guy to writeCoursesToFitnessDevice
     // TODO Add generic fitness write method
	...
	/** Writes the given ActivityGoals TCX string to the device selected in this.deviceNumber. <br/>
	 * <br/>
	 * Minimum plugin version 2.2.0.1
	 * 
     ...
     * @param fileName {String} filename to write data to on the device.  Validity is not checked here.
     */	
	writeGoalsToFitnessDevice: function(tcxString, fileName) {
		if (!this.isUnlocked())
			throw new Error(Garmin.DeviceControl.MESSAGES.pluginNotUnlocked);
		if(this.numDevices == 0)
			throw new Error(Garmin.DeviceControl.MESSAGES.noDevicesConnected);
		this.gpsDataType = Garmin.DeviceControl.FILE_TYPES.goals;
		try {
        	this._broadcaster.dispatch("onStartWriteToDevice", {controller: this});
		    this.garminPlugin.startWriteFitnessData(tcxString, this.deviceNumber, fileName, this.gpsDataType);
		    this._progressWrite();
        ...
        try {
			...
			switch( this.gpsDataType ) {
				...
				case Garmin.DeviceControl.FILE_TYPES.binary :
					...
					break;
				default:
					throw new Error("Unsupported datatype to write to device.");
			}
